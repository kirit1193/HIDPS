import fcntl, easygui, logging, time, urllib, urllib2, sys, simplejson, ttk, multiprocessing, Queue, os
from Tkinter import *
...
def optionsON():
	...
	whiteButton=Button(listframe,text="Clear Whitelist")
	whiteButton.grid(row=0, column=0, ipadx=10,pady=7)
	blackButton=Button(listframe,text="Clear Blacklist")
	blackButton.grid(row=0, column=2, ipadx=10,pady=7)
	greyButton=Button(listframe,text="Clear Greylist")
...
def parseClick ():
	global thrVT
	parse_win.deiconify()
	thrVT = Thread(target=vtLookup, args=("www.isoeh.com",))
	thrVT.start()
	
def vtLookup (testIP):
	url = "https://www.virustotal.com/vtapi/v2/url/scan"
	#testIP = "http://www.isoeh.com"
	...
	if rating < 1:
		text_file = open('whitelist.txt', 'a')
		text_file.write(str(testIP))
		text_file.write("\n")
		text_file.close()
	elif rating < 5:
		text_file = open('greylist.txt', 'a')
		text_file.write(str(testIP))
		text_file.write("\n")
		text_file.close()
	else:
...
def mainUI():
	global main_win, border_in, t, border_out, t1, border_misc, t2, border_buttons, border_buttons1, border_buttons2, countinPackets, countoutPackets, inpCount, outpCount, aboutbutton, quitButton, viewip, parseButton, screen_height, screen_width,pauseButton
	main_win = Tk()
	...
	menu.add_cascade(label="File",menu=fileMenu,activeforeground="green")
	...
	menu.add_cascade(label="Options",menu=optionMenu,activeforeground="green")
	optionMenu.add_cascade(label="Advanced Options",menu=Omenu,activeforeground="green")
	...
	photo1=ImageTk.PhotoImage(image1)
	quitButton=Button(border_buttons,image=photo1,width=70,height=32,command=lambda:exitClick(main_win))
	quitButton.image=photo1
	quitButton.pack(side="left",ipadx=10,pady=7)
